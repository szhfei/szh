...
% iend = istart + 120000-1;
% 
% lll = length(sch1) + length(Alpha_ch1) - 1;
% %地表相位
% fsch1 = fft(sch1,lll);
% phase = fsch1 ./ abs(fsch1);
% % %地铁相位
% % fuch1 = fft(uch1,lll);
% % phase = fuch1 ./ abs(fuch1);
% 
% sch_left = myfreq(uch1,sch1,fftshift(Alpha_ch1),phase);
% u = uch1(istart:iend);
% s = sch1(istart:iend);
...
% % end
% figure(100)
% plot(f,abs(alpha_ch1),f,abs(alpha_left_ch1{1}),f,abs(alpha_left_ch1{2}),f,abs(alpha_left_ch1{3}),f,abs(alpha_left_ch1{4})),xlim([0.1 25])...
...
% sch_other{i} = myfreq(uch1,sch1,fftshift(Atest{i}),phase);
% res{i} = sch_other{i}(istart:iend);
% end
% 
% 
...
% 
% figure(1001)
% plot(f,abs(alpha_ch1),f,abs(test{aaa})),xlim([.1 25]),xlabel('f/Hz','fontsize',22),ylabel('幅值','fontsize',22)...
